...
public struct XIPFile {
            ...
            case C_ISREG:
                ...
                hardlinks[file.identifier] = (
                    ...
                    taskStream.addRunningTask {
                        await task?.value
                        let compressedData = await file.compressedData()
    ...
    public func expand(to destination: URL) async throws {
        ...
        guard chdir(destination.path) == 0 else {
            fputs("Failed to access output directory at \(destination.path): \(String(cString: strerror(errno)))", stderr)
            exit(EXIT_FAILURE)
        }
